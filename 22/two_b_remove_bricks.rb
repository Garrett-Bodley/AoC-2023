...
class Solver # rubocop:disable Metrics/ClassLength
  ...
  def drop_again(bricks, matrix)
    bricks.sort { |a, b| a.z.begin <=> b.z.begin }.reduce(0) do |sum, brick|
      sum + try_drop(brick, matrix)
    end
  end

  def try_drop(brick, matrix)
    z_max = 0
    brick.y.each do |y|
      brick.x.each do |x|
        z_max = matrix[y][x] if matrix[y][x] > z_max
      end
    end
    raise Hm unless z_max < brick.z.begin

    z_offset = brick.z.begin - z_max - 1
    dropped = z_offset == 0 ? false : true

    brick.z = (brick.z.begin - z_offset)..(brick.z.end - z_offset)

    brick.y.each do |y|
      brick.x.each do |x|
        matrix[y][x] = brick.z.end
      end
    end
  ...
  def solve
    ...
    max_dropped = @bricks.map do |brick|
      matrix = build_height_matrix
      removed = @bricks.reject { |br| br == brick }.map(&:dup)
      drop_again(removed, matrix)
    end
    max_dropped.max + 1
  end
